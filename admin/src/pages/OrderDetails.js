  ...
  const renderAddress = () => {
    ...
    return (
      <div className="address-details">
        <p className="address-name"><strong>{addr.fullName || "Guest"}</strong></p>
        <p className="address-line">{addr.houseNo || ""}</p>
        <p className="address-line">{addr.area || ""}</p>
        {addr.landmark && <p className="address-line">Landmark: {addr.landmark}</p>}
      ...
      <div className="order-details-content">
        ...
        <div className="order-details-left">
          ...
          <div className="detail-card status-card">
            ...
            <div className="status-timeline">
              <div className={`timeline-item ${order.orderStatus === 'Placed' || true ? 'active' : ''}`}>
              ...
              <div className={`timeline-item ${['Accepted', 'Packed', 'Out for Delivery', 'Delivered'].includes(order.orderStatus) ? 'active' : ''}`}>
                <div className="timeline-dot"></div>
                <div className="timeline-content">
                  <span className="timeline-title">Order Accepted</span>
                </div>
              </div>
              <div className={`timeline-item ${['Packed', 'Out for Delivery', 'Delivered'].includes(order.orderStatus) ? 'active' : ''}`}>
                <div className="timeline-dot"></div>
                <div className="timeline-content">
                  <span className="timeline-title">Order Packed</span>
                </div>
              </div>
              <div className={`timeline-item ${['Out for Delivery', 'Delivered'].includes(order.orderStatus) ? 'active' : ''}`}>
                <div className="timeline-dot"></div>
                <div className="timeline-content">
        ...
        <div className="order-details-right">
          ...
          <div className="detail-card">
            <div className="card-header">
              <h3><FaRupeeSign /> Payment Details</h3>
            </div>
          ...
          <div className="detail-card">
            ...
            <div className="info-list">
              <div className="info-item">
                ...
                <span className="info-value">{order.address?.fullName || "Guest"}</span>
              </div>
              <div className="info-item">
                <span className="info-label"><FaEnvelope /> Email</span>
                <span className="info-value">{order.user?.email || order.address?.email || "N/A"}</span>
              </div>
              <div className="info-item">
                <span className="info-label"><FaPhone /> Phone</span>
                <span className="info-value">{order.address?.mobileNumber || "N/A"}</span>
              </div>
            </div>
          </div>
